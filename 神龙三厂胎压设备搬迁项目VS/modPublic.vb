...
hasDSG_Err:
    ...
    Public Function SetProNum(ByRef ProNum As String) As Object
        ...
        'UPGRADE_WARNING: 未能解析对象 oRVT520.SendProNum 的默认属性。 单击以获得更多信息:“ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"”
        oRVT520.SendProNum(CShort(ProNum))
        'UPGRADE_WARNING: 未能解析对象 oLVT520.SendProNum 的默认属性。 单击以获得更多信息:“ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"”
...
getConfigValue_err:
    ...
    ' 打开串口
    Public Sub OpenSerialPort(serialPort As SerialPort, portName As String, setting As String, name As String)
        Try
            Dim sets As String() = setting.Split(",")
            serialPort.PortName = "COM" + portName
            serialPort.BaudRate = Int32.Parse(sets(0))
            Select Case sets(1)
                Case "n"
            ...
            End Select
            serialPort.DataBits = Int32.Parse(sets(2))
            serialPort.StopBits = sets(3)
            serialPort.Open()
        Catch ex As Exception
            log.LogError($"{name}打开串口失败：{serialPort.PortName}", ex)
        End Try
    End Sub
